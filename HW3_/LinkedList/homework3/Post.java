...
public class Post
{
    ...
    private  LinkedList<Like> like_array=new LinkedList<Like>() ;
    ...
    public boolean increase_like(String user_name)
    {
        ...
        try {
            ListIterator<Like> iter=like_array.listIterator();
            while(iter.hasNext())
            {
                Like temp_like=iter.next();
                if(user_name.equals(temp_like.getUSer_name()))
    ...
    public boolean delete_like(String user_name)
    {
        try {
            ...
            // {
            //     if(user_name.equals(like_array.get(i).getUSer_name()))/*find like that wanted remove */
            //     {
            //         like_array.remove(like_array.get(i));
            //         // Like [] new_like_array= new Like[like_number-1];
            //         // int counter=-1;
      ...
      public boolean delete_comment(String content,int post_ID,String user)
      {
          try {
              if(post_ID==post_id)
              {
                ...
                while (iter.hasNext()) 
                {
                    Comment temp_comment=iter.next();
                    if(temp_comment.get_Comment().equals(content)&&temp_comment.getUSer_name().equals(user))
                    {
                        iter.remove();
                        return true;
                    }
