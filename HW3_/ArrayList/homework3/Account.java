...
public class Account {
    ...
    private ArrayList<String> Ban= new ArrayList<String>();
    private ArrayList<Message> Inbox= new ArrayList<Message>();
    private ArrayList<Message> out_BOX= new ArrayList<Message>();
    ...
    public String toString() {
        ...
        for(int i=0;i<follower.size();i++)
            followerr+=follower.get(i)+" ";
        String followingg="";
        
        for(int i=0;i<following.size();i++)
            followingg+=following.get(i)+" ";
    ...
    */
    ...
    private boolean add_follower(Account temp)
    {
        ...
        try {
            if(temp!=null)
            {
                ...
                follower.add(temp.getUSER_NAME());//add account's user name that has been follow this account in follower list 
    ...
    private boolean delete_follower(Account temp)
    {
        ...
        try {
            ...
            {
                temp.login();
                if(follower.indexOf(temp.getUSER_NAME())!=-1)//find temp's userName in follower list
                {
                    follower.remove(temp.getUSER_NAME());//remove temp's user name from follower list
                    if(temp.delete_following(this))//remove owner of account's user name Temp's following list
                    {
                        ...
                        temp.logout();//logout to temp account
                        return true;
                    }
                }
    ...
    */
    public boolean new_following(Account temp)
    {
        ...
        try 
        {
            if(temp!=null)
            {
                ...
                {
                    following.add(temp.getUSER_NAME());
    ...
    public boolean delete_following(Account temp)
    {
        ...
        try {
            ...
            {
                temp.login();
                if(following.indexOf(temp.getUSER_NAME())!=-1)//find temp's user name in following list
                {
                    following.remove(temp.getUSER_NAME());//remove it from following list
    ...
    public void new_post(Post new_post)
    {
        ...
        Posts.add(new_post);//add new post to post list
    }
    ...
    public boolean delete_ban(Account temp)
    {
        ...
        try {
            ...
            {
                temp.login();
                if(Ban.contains(temp.getUSER_NAME()))
                {
                    Ban.remove(temp.getUSER_NAME());
    ...
    public void get_post_information(Account temp)
    {
        if(!is_loge)
        {
            ...
            if(temp!=null)
            {
                ...
                {
                    System.out.println("Post ID :"+i+" "+USER_NAME+": " +temp.Posts.get(i).get_content_of_post()+" \n ");
                }
    ...
    public void get_interaction_information(Account temp)
    {
        ...
        try {
            ...
            if(temp!=null)
            {
                ...
                int j=0;
                int counter=0;
                while(j<temp.Posts.size())
                {
                    Post temp_post=temp.Posts.get(j);
                    System.out.println("---------------------------------------");
                    ...
                    System.out.println("---------------------------------------");
                    // ListIterator<Like> iter2=temp_post.Like
                    j++;
                }
    ...
    /**
     * this class represent for the  return user history
